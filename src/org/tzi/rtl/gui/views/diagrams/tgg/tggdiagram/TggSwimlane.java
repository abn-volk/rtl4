...
public class TggSwimlane {
	...
	public TggSwimlane(TggDiagramView tggDiagramView, int x1, int x2, int x3, int x4, int y1, int y2, int y3, int y4){
        ...
        if(x1 == x2) fSource_Border = null;
        if(y1== y2) fTopTggLine = null;
        if(y3 == y4) fBottomLine = null;
	}
	...
	public void moveSwimlane(int draggingSwimlane, int dx, int dy) {
		...
		case 1: //fSource_Border;		
			...
			if( (0 < p1.getX() + dx) && (p1.getX() + dx < fSource_Corr.getX1())  ){				
				...
				if(dx > 0){
					List objects = this.fTggDiagramView.getTggRule().getSourceRule().getAllObjects();
		...
		case 2: //fSource_Corr;
			...
			if((getMinX() < p1.getX() + dx) && (p1.getX() + dx < fTarget_Corr.getX1())){
				...
				List objects;
				if(dx<0){
					objects = this.fTggDiagramView.getTggRule().getSourceRule().getAllObjects();
				} else{
		...
		case 3: //fTarget_Corr;
			...
			if((fSource_Corr.getX1() < p1.getX() + dx) && (p1.getX() + dx < fTarget_Border.getX1())){
				...
				if(dx < 0){
					...
					objects = this.fTggDiagramView.getTggRule().getTargetRule().getAllObjects();
				}
				boolean ok = tryMoveLeftRightObjects(objects, dx);			
				if (!ok){
					p1.setLocation(p1.getX() - dx, p1.getY());
		...
		case 4: //fTarget_Border;			
			...
			if(fTarget_Corr.getX1() < p1.getX() + dx){
				...
				boolean ok = true;
				if(dx < 0){
					List objects = this.fTggDiagramView.getTggRule().getTargetRule().getAllObjects();
				...
				if(ok){
					if(fTopTggLine != null){
						fTopTggLine.setLine(fTopTggLine.getX1(), fTopTggLine.getY1(), fTopTggLine.getX2() + dx, fTopTggLine.getY2());
					}
					fMiddleTggLine.setLine(fMiddleTggLine.getX1(), fMiddleTggLine.getY1(), fMiddleTggLine.getX2() + dx, fMiddleTggLine.getY2());
					fBottomTggLine.setLine(fBottomTggLine.getX1(), fBottomTggLine.getY1(), fBottomTggLine.getX2() + dx, fBottomTggLine.getY2());
					if(fBottomLine!=null){
						fBottomLine.setLine(fBottomLine.getX1(), fBottomLine.getY1(), fBottomLine.getX2() + dx, fBottomLine.getY2());
					}
				}
			}
			break;
		case 10: //fTopTggLine;
			if(fTopTggLine == null) break;
			...
			if( (0 < p1.getY() + dy) && (p1.getY() + dy < fMiddleTggLine.getY1()) ){
				...
				Set objects = this.fTggDiagramView.getTggRule().getSystemState_LHS().allObjects();
		...
		case 20: //fMiddleTggLine;
			...
			if( (getMinY() < p1.getY() + dy) && (p1.getY() + dy < fBottomTggLine.getY1()) ){
				...
				Set objects;
				if (dy < 0){
					objects = this.fTggDiagramView.getTggRule().getSystemState_LHS().allObjects();
				}else{
					objects = this.fTggDiagramView.getTggRule().getSystemState_RHS().allObjects();
				}
				boolean ok = tryMoveUpDownObjects(objects, dy);			
				if (!ok){
					p1.setLocation(p1.getX(), p1.getY() - dy);
		...
		case 30: //fBottomTggLine;
			...
			if( (fMiddleTggLine.getY1() < p1.getY() + dy) && (p1.getY() + dy < getMaxY()) ){
				...
				boolean ok = true;
				if(dy < 0){
					Set objects = this.fTggDiagramView.getTggRule().getSystemState_RHS().allObjects();
				...
				if(ok && (fBottomLine == null) ){
					if(fSource_Border != null){
						fSource_Border.setLine(fSource_Border.getX1(), fSource_Border.getY1(), fSource_Border.getX2(), fSource_Border.getY2() + dy);
					}
					fSource_Corr.setLine(fSource_Corr.getX1(), fSource_Corr.getY1(), fSource_Corr.getX2(), fSource_Corr.getY2() + dy);	
					fTarget_Corr.setLine(fTarget_Corr.getX1(), fTarget_Corr.getY1(), fTarget_Corr.getX2(), fTarget_Corr.getY2() + dy);
					fTarget_Border.setLine(fTarget_Border.getX1(), fTarget_Border.getY1(), fTarget_Border.getX2(), fTarget_Border.getY2() + dy);		      
				}
			}
			break;
		case 40: //fBottomLine;
			if(fBottomLine == null) break;
	...
	public boolean tryMoveLeftRightObjects(List objects, int dx){
		boolean res = true;
		List movedObjects = new ArrayList();
		MObject mObject;
		Iterator it = objects.iterator();
		while(it.hasNext()){
			mObject = (MObject) it.next();
			ObjectNode node = fTggDiagramView.getTggDiagram().getObjectNode(mObject);
			int gap = dx>0?10:-10;
	...
	public boolean tryMoveUpDownObjects(Set objects, int dy){
		boolean res = true;
		List movedObjects = new ArrayList();
		Iterator it = objects.iterator();
		while(it.hasNext()){
			ObjectNode node = fTggDiagramView.getTggDiagram().getObjectNode((MObject) it.next());
			int gap = dy>0?10:-10;
