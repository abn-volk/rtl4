...
//      Add the scroll panes to a split pane.
    ...
    private void initState() {    	
    	MRule sourceRule = fTggRule.getSourceRule();
    	MRule targetRule = fTggRule.getTargetRule();
    ...
    void displayOCLConditions(){
    	...
    	conditions = fTggRule.getCorrRule().preconditions();
    	setTextHtmlPane(fTggCL_HtmlPane, conditions);
    	conditions = fTggRule.getCorrRule().postconditions();
    	setTextHtmlPane(fTggCR_HtmlPane, conditions);
    	
    	conditions = fTggRule.getTargetRule().preconditions();
    	setTextHtmlPane(fTggTL_HtmlPane, conditions);
    	conditions = this.fTggRule.getTargetRule().postconditions();
    ...
    boolean hasLinkBetweenObjects(MAssociation assoc, MObject[] objects) {
       return fTggRule.getSystemState_RHS().hasLinkBetweenObjects(assoc, objects);
    }
