...
import org.tzi.rtl.gui.views.diagrams.tgg.*;
import org.tzi.rtl.gui.views.diagrams.tgg.event.*;
...
public class TggDiagram extends DiagramView {
    ...
    public static class TggDiagramData {
		...
		public ObjectNode getObjectNode(MObject obj){
    ...
    public void addObject(MObject obj, int groupId) {
        ...
        ObjectNode n = new ObjectNode( obj, groupId, fParent, fOpt);
    ...
    public ObjectNode getObjectNode(MObject mObject){
    	return (ObjectNode)this.tggDiagramData.fObjectToNodeMap.get(mObject);
    }
	...
	protected void storePlacementInfos(PersistHelper helper, Element root, boolean visible) {
		...
		for (ObjectNode n : tggDiagramData.fObjectToNodeMap.values()) {
	...
	public void restorePlacementInfos(PersistHelper helper,
		...
		for (int i = 0; i < elements.getLength(); ++i) {
			...
			MObject obj = fParent.getTggRule().getSystemState_RHS().system().state().objectByName(name);
			// Could be deleted
			if (obj != null) {
				ObjectNode node = tggDiagramData.fObjectToNodeMap.get(obj);
		...
		for (int i = 0; i < elements.getLength(); ++i) {
			...
			MAssociation assoc = fParent.getTggRule().getSystemState_RHS().system().model().getAssociation(name);
			...
			MObject sourceObject = fParent.getTggRule().getSystemState_RHS().system().state().objectByName(sourceObjectName);
    ...
    private void displayObjectInfo(MObject obj, MouseEvent e) {
        ...
        MObjectState objState = obj.state(fParent.getTggRule().getSystemState_RHS().system().state());
    ...
    protected MLink getLinkByValue(MAssociation assoc, List<MObject> objects, String linkValue) {
		Set<MLink> links = fParent.getTggRule().getSystemState_RHS().system().state().linkBetweenObjects(assoc, objects);
		if (links.size() == 1) {
